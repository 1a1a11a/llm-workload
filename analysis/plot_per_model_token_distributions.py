#!/usr/bin/env python3
"""
Token usage distribution analysis script for LLM workload metrics.

This script analyzes input and output token distributions across different models.
It reads per-model CSV files and generates comprehensive statistical visualizations
including CDF plots, frequency distributions, and ratio analyses.

Key Features:
- Analyzes input tokens, output tokens, and output/input ratio distributions
- Supports parallel processing for multiple model files
- Generates both combined and individual model plots
- Uses log-scale visualization for wide value ranges
- Provides statistical summaries in plots

Input Requirements:
- Per-model CSV files containing token data
- Expected data columns: input_tokens, output_tokens
- Files should be located at:
  /scratch/juncheng/data/prefix_cache/data/metrics_30day/per_model/

Output:
- Combined CDF plots for all models
- Individual model plots
- Statistical summaries in plot annotations
- All plots saved to figures/per_model_token_distributions/

Usage:
    python3 plot_per_model_token_distributions.py model1.csv model2.csv [model3.csv ...]

Example:
    # Analyze top 10 models by request count
    ...
    echo "$traces"
    python3 plot_per_model_token_distributions.py $traces
...
def main(
    ...
    output_path = Path(f"/scratch/juncheng/data/prefix_cache/{output_dir}/figures/per_model_token_distributions/{trace_name}")
